...
public abstract class PositionalLight extends Light {
	...
	void update () {
		if (body != null && !staticLight) {
			...
			if(useBodyAngle) {
				...
				angle = direction * MathUtils.degreesToRadians;
			}
	...
	public PositionalLight (RayHandler rayHandler, int rays, Color color, float distance, float x, float y, float directionDegree, boolean useBodyAngle) {
		super(rayHandler, rays, color, directionDegree, distance);
		this.useBodyAngle = useBodyAngle;
		start.x = x;
		start.y = y;
		sin = new float[rays];
		cos = new float[rays];
